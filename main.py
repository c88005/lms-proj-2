...
class Engine(arcade.Window):
    ...
    def upgrade_weapon(self):
        if self.player.scrap_parts >= self.shop_weapon_cost and self.player.weapon != 6:
            ...
            if self.player.weapon != 6:
                self.shop_weapon_cost *= 4
            else:
    ...
    def draw_menu(self):
        start_button = self.gui.create_text(f"Нажмите Пробел чтобы начать", 405, 410, arcade.color.WHITE, 24, 160)
    ...
    def on_key_press(self, key, modifiers):
        ...
        if key == arcade.key.RSHIFT:
            if self.screenshotting:
                if self.screenshotting == False: return
                self.screenshotting = False
            else:
                if self.screenshotting == True: return
